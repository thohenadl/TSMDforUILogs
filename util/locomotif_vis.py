...
def plot_motif_sets(series, motif_sets, gt=None, dimension_names=None, legend=True):
    ...
    for i, motif_set in enumerate(motif_sets):
        ...
        avg_distance = sum(end - start for start, end in motif_set) / len(motif_set)
        axs[i+1].set_title(f"Motif Set {i+1}, k: {len(motif_set)}, Avg. Distance: {avg_distance}", fontsize=BIG_SIZE)
